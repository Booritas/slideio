...
void compressRasterToFile(const cv::Mat& mat, const std::string& targetPath, const ConvertJ2KParameters& params)
{
    ...
    if (!stream.get()) RAISE_RUNTIME_ERROR << "Cannot create default file stream.";
    if (!opj_start_compress(codec, image, stream))
        RAISE_RUNTIME_ERROR << "Failed to encode image : opj_start_compress.";
    if (!opj_encode(codec, stream)) RAISE_RUNTIME_ERROR << "Failed to encode image : opj_encode.";
    if (!opj_end_compress(codec, stream)) RAISE_RUNTIME_ERROR << "Failed to encode image : opj_end_compress.";
}
...
void compress(const std::string& sourcePath, const std::string& driver, const std::string& targetPath,
              const ConvertJ2KParameters& params)
{
