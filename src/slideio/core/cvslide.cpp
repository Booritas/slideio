...
MetadataFormat CVSlide::recognizeMetadataFormat(const std::string& metadata)
{
    ...
    if (trimmed.empty()) return MetadataFormat::None;

    if (trimmed[0] == '{' || trimmed[0] == '[') return MetadataFormat::JSON;

    if (trimmed.size() >= 5 && trimmed.substr(0, 5) == "<?xml") return MetadataFormat::XML;
    if (trimmed[0] == '<') return MetadataFormat::XML;
