...
namespace slideio
{
    ...
    public:
        ...
        friend Range operator+(const Range& r, int32_t delta) { return Range(r.start + delta, r.end + delta); }

        friend Range operator+(int32_t delta, const Range& r) { return Range(r.start + delta, r.end + delta); }

        friend Range operator-(const Range& r, int32_t delta) { return r + (-delta); }
