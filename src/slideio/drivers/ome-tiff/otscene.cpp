...
void OTScene::extractMagnificationFromMetadata()
{
    ...
    if (xmlInstrumentRef) instrumentRef = xmlInstrumentRef->Attribute("ID");
    tinyxml2::XMLElement* xmlObjectiveRef = m_imageXml->FirstChildElement("ObjectiveSettings");
    const char* objectiveRef = nullptr;
    if (xmlObjectiveRef) objectiveRef = xmlObjectiveRef->Attribute("ID");
    if (instrumentRef != nullptr && objectiveRef != nullptr)
    {
        tinyxml2::XMLElement* xmlRoot = m_imageDoc->RootElement();
        for (tinyxml2::XMLElement* xmlInstrument = xmlRoot->FirstChildElement("Instrument"); xmlInstrument != nullptr;
             xmlInstrument = xmlInstrument->NextSiblingElement("Instrument"))
        {
            ...
            if (id != nullptr && strcmp(id, instrumentRef) == 0)
            {
                for (tinyxml2::XMLElement* xmlObjective = xmlInstrument->FirstChildElement("Objective");
                     xmlObjective != nullptr; xmlObjective = xmlObjective->NextSiblingElement("Objective"))
                {
    ...
    else
    {
        ...
        if (xmlInstrument != nullptr)
        {
            ...
            if (xmlObjective) m_magnification = xmlObjective->DoubleAttribute("NominalMagnification", 0);
        }
    }
...
void OTScene::extractTiffData(tinyxml2::XMLElement* pixels)
{
    ...
    for (tinyxml2::XMLElement* xmlTiffData = pixels->FirstChildElement("TiffData"); xmlTiffData != nullptr;
         xmlTiffData = xmlTiffData->NextSiblingElement("TiffData"))
    {
        try
        {
            TiffData tiffData;
            tiffData.init(m_filePath, &m_files, m_dimensionOrder, m_numChannels, m_numZSlices, m_numTFrames,
                          xmlTiffData);
    ...
    if (m_tiffData.empty()) RAISE_RUNTIME_ERROR << "OTScene: no valid TiffData elements in the xml metadata";
...
void OTScene::extractImagePyramids()
{
    ...
    SLIDEIO_LOG(INFO) << "OTScene: Image pyramids successfully extracted for image: " << m_imageId
                      << ". Number of levels: " << m_levels.size();
}

void OTScene::initialize()
{
    SLIDEIO_LOG(INFO) << "OTScene: Initializing scene for image: " << m_imageId;
    if (m_imageXml == nullptr || m_imageDoc == nullptr) RAISE_RUNTIME_ERROR << "OTScene: Image xml is not set";
    ...
    if (!pixels) RAISE_RUNTIME_ERROR << "OTScene; missing required Pixels element in the xml metadata";
    const char* dimOrder = pixels->Attribute("DimensionOrder");
    if (!dimOrder) RAISE_RUNTIME_ERROR << "OTScene: missing required DimensionOrder attribute in the xml metadata";
    m_dimensionOrder = dimOrder;
    const char* pixelType = pixels->Attribute("Type");
    if (!pixelType) pixelType = pixels->Attribute("PixelType");
    if (!pixelType) RAISE_RUNTIME_ERROR << "OTScene: missing required Type attribute in the xml metadata";
    m_dataType = OTTools::stringToDataType(pixelType);
    if (m_dataType == DataType::DT_Unknown) RAISE_RUNTIME_ERROR << "OTScene: unsupported pixel type: " << pixelType;
    m_numChannels = pixels->IntAttribute("SizeC", 0);
    if (m_numChannels <= 0) RAISE_RUNTIME_ERROR << "OTScene: invalid number of channels: " << m_numChannels;
    m_numZSlices = pixels->IntAttribute("SizeZ", 0);
    if (m_numZSlices <= 0) RAISE_RUNTIME_ERROR << "OTScene: invalid number of z-slices: " << m_numZSlices;
    m_numTFrames = pixels->IntAttribute("SizeT", 0);
    if (m_numTFrames <= 0) RAISE_RUNTIME_ERROR << "OTScene: invalid number of time frames: " << m_numTFrames;
    ...
    if (m_bigEndian) SLIDEIO_LOG(INFO) << "OTScene: BigEndian pixel order is declared in metadata";

    m_zResolution = pixels->DoubleAttribute("PhysicalSizeZ", 0.0);
    const char* att = pixels->Attribute("PhysicalSizeZUnit");
    if (att != nullptr) m_zResolution = OTTools::convertToMeters(m_zResolution, att);

    m_tResolution = pixels->DoubleAttribute("PhysicalSizeT", 0.0);
    att = pixels->Attribute("PhysicalSizeZUnit");
    if (att != nullptr) m_tResolution = OTTools::convertToSeconds(m_tResolution, att);
    ...
    if (!resolutionProcessed) m_resolution = dir.res;
...
void OTScene::initializeChannelAttributes(tinyxml2::XMLElement* pixels)
{
    ...
    int channelIndex = 0;
    for (tinyxml2::XMLElement* xmlChannel = pixels->FirstChildElement("Channel"); xmlChannel != nullptr;
        ...
        if (channelIndex < m_numChannels)
        {
            ...
            while (attribute != nullptr)
            {
                ...
                if (attrName != nullptr && attrValue != nullptr)
                {
                    ...
                    {
                        if (m_channelNames.empty()) m_channelNames.resize(m_numChannels);
    ...
    SLIDEIO_LOG(INFO) << "OTScene: Channel names initialized for image: " << m_imageId
                      << " channels: " << m_channelNames;
}
...
int OTScene::findZoomLevel(double zoom) const
{
    ...
    int index = Tools::findZoomLevel(zoom, (int)m_levels.size(), [&levels, sceneWidth](int index) {
...
void OTScene::collectTiffDataIndices(std::vector<int> channelIndices, int zSliceIndex, int tFrameIndex,
                       ...
                       void* userData)
{
    ...
    for (const cv::Mat& channelRaster : channelRasters)
    {
        ...
        {
            RAISE_RUNTIME_ERROR << "OMETIFF driver: empty raster for tile index: " << tileIndex
                                << " channel: " << channelIndices[channel];
        }
        ++channel;
    }
...
void OTScene::initializeBlock(const cv::Size& blockSize, const std::vector<int>& channelIndices, cv::OutputArray output)
{
