...
void TiffData::init(const std::string& filePath, TIFFFiles* files, const std::string& dimOrder, int numChannels,
                    int numZSlices, int numTFrames, tinyxml2::XMLElement* xmlTiffData)
{
    ...
    if (xmlTiffData == nullptr) RAISE_RUNTIME_ERROR << "TiffData: Unexpected xmlTiffData is null";
    if (directoryPath.empty()) RAISE_RUNTIME_ERROR << "TiffData: Unexpected empty directory path";
    if (files == nullptr) RAISE_RUNTIME_ERROR << "TiffData: Unexpected TIFFFiles collection is null";
    ...
    if (!m_tiff) RAISE_RUNTIME_ERROR << "OTScene: cannot open file " << m_filePath << " with libtiff";
    ...
    m_coordinatesFirst =
        m_dimensions.createCoordinates({{DimC, firstChannel}, {DimZ, firstZSlice}, {DimT, firstTFrame}});
...
{
    const std::list<std::pair<std::string, int>> coordList = {{DimC, channel}, {DimZ, slice}, {DimT, frame}};
...
{
    if (plane < 0 || plane >= m_planeCount) RAISE_RUNTIME_ERROR << "TiffData: Invalid plane index: " << plane;
    return m_directories[plane];
}

void TiffData::readTile(const std::vector<int>& channelIndices, int zSlice, int tFrame, int zoomLevel, int tileIndex,
                        std::vector<cv::Mat>& rasters) const
{
    ...
    if (myChannelIndices.empty()) return;
    ...
    for (int plane = 0; plane < m_planeCount; ++plane)
    {
        ...
        const TiffDirectory& dir =
            (zoomLevel == 0) ? m_directories[plane] : m_directories[plane].subdirectories[zoomLevel - 1];
        cv::Mat localRaster;
        ...
        if (myChannelIndices.empty()) break;

        m_dimensions.incrementCoordinates(coords);
    }

    if (!myChannelIndices.empty()) RAISE_RUNTIME_ERROR << "TiffData: unexpected block: no channels were read";
}
