...
void OTDimensions::init(const std::string& dimensionOrder, int numChannels, int numZSlices, int numTFrames,
                        int samplesPerPixel)
{
    if (dimensionOrder.size() != 5) RAISE_RUNTIME_ERROR << "OTScene: unexpected dimension order: " << dimensionOrder;
    if (numChannels <= 0 || numZSlices <= 0 || numTFrames <= 0)
    {
        RAISE_RUNTIME_ERROR << "OTScene: unexpected dimension size: " << numChannels << ", " << numZSlices << ", "
                            << numTFrames;
    }
    if (samplesPerPixel <= 0) RAISE_RUNTIME_ERROR << "OTScene: unexpected samples per pixel: " << samplesPerPixel;
