...
{
    if (index != 0) throw std::runtime_error("ZVIImageDriver: Invalid scene index");
    return m_scene;
}
...
namespace
{
    ...
    nlohmann::json tagsToJsonObject(const std::vector<ZVIUtils::ZviTagEntry>& entries)
    {
        ...
        for (const auto& e : entries)
        {
            ...
            const std::string key = name ? std::string(name) : "Tag_" + std::to_string(e.id);
    ...
    void hoistInvariantTags(nlohmann::json& parent, std::vector<nlohmann::json*>& children)
    {
    ...
    std::optional<nlohmann::json> tryReadTagsObject(ole::compound_document& doc, const std::string& path,
                                                    bool hasClsidHeader)
    {
        try
        {
...
void ZVISlide::init()
{
    ...
    for (const ZVIImageItem& item : scene->getImageItems())
    {
        ...
        const std::string path = "/Image/Item(" + std::to_string(item.getItemIndex()) + ")/Tags/Contents";
        ...
        for (int c = 0; c < numChannels; ++c)
        {
                    ...
                    if (zIt != channelIt->second.end())
                    {
                        if (numTFrames > 1)
                        {
                            ...
                            for (int t = 0; t < numTFrames; ++t)
                            {
                                ...
                                tArr.push_back(tIt != zIt->second.end() ? std::move(tIt->second) :
                                                                          nlohmann::json::object());
                            }
                            for (auto& tElem : tArr)
                                tPtrs.push_back(&tElem);
                        ...
                        else
                        {
                            ...
                            if (tIt != zIt->second.end()) zObj = std::move(tIt->second);
                        }
                    }
            ...
            else
            {
                ...
                if (zIt != channelIt->second.end())
                {
                    ...
                    {
                        auto tIt = zIt->second.find(0);
                        if (tIt != zIt->second.end()) channelObj = std::move(tIt->second);
                    }
                }
            }
...
{
    if (m_rawMetadata.empty()) return CVSlide::buildMetadataTree();
