...
namespace slideio
{
        ...
        public:
            ...
            // Likewise for the tile position: ZVIScene::computeTiles() resolves
            // an unknown one against the grid. It has to be written back to the
            // item, because ZVITile::addItem() reads the position from the item
        ...
        private:
            ...
            int m_SceneIndex = -1;
            // -1 means "the tag stream did not say". ZVIScene::computeTiles()
            // resolves that to (0,0) only when the image is a single tile;
            // guessing a position on a mosaic serves the wrong pixels.
            int m_TileIndexX = -1;
            int m_TileIndexY = -1;
            int m_ValidBits = 0;
