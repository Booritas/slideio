...
bool slideio::CZISubBlock::isInBlock(int channel, int z, int t, int r, int s, int i, int b, int h, int v) const
{
    const bool inBlock = (channel >= firstChannel() && channel <= lastChannel()) &&
                         (z >= firstZSlice() && z <= lastZSlice()) && (t >= firstTFrame() && t <= lastTFrame()) &&
                         (r >= firstRotation() && r <= lastRotation()) && (s >= firstScene() && s <= lastScene()) &&
                         (i >= firstIllumination() && i <= lastIllumination()) &&
                         (b >= firstBAcquisition() && b <= lastBAcquisition()) &&
                         (h >= firstHPhase() && h <= lastHPhase()) && (v >= firstView() && v <= lastView());
    return inBlock;
}

slideio::CZISubBlock::CZISubBlock()
    : m_dataType(DataType::DT_Unknown), m_cziPixelType(0), m_pixelSize(1), m_planeSize(0), m_filePosition(-1),
      m_dataPosition(-1), m_dataSize(0), m_filePart(-1), m_compression(-1), m_channelIndex(-1), m_zSliceIndex(-1),
      m_tFrameIndex(-1), m_illuminationIndex(-1), m_bAcquisitionIndex(-1), m_rotationIndex(-1), m_sceneIndex(-1),
...
int64_t slideio::CZISubBlock::computeDataOffset(int channel, int z, int t, int r, int s, int i, int b, int h,
                                                int v) const
{
    if (!isInBlock(channel, z, t, r, s, i, b, h, v)) return -1;
    ...
    const int64_t offset = (channel - firstChannel()) * channelSize + (z - firstZSlice()) * zSize +
                           (t - firstTFrame()) * tSize + (r - firstRotation()) * rSize + (s - firstScene()) * sSize +
                           (i - firstIllumination()) * iSize + (b - firstBAcquisition()) * bSize +
                           (h - firstHPhase()) * hSize + (v - firstView()) * vSize;
    return offset;
}

void slideio::CZISubBlock::setupBlock(const SubBlockHeader& subblockHeader,
                                      std::vector<DimensionEntryDV>& dimensionEntries)
{
