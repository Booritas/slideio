...
void slideio::NDPIFile::init(const std::string& filePath)
{
    ...
    if (!m_tiff.isValid()) RAISE_RUNTIME_ERROR << "NDPIImageDriver: Cannot open file:" << filePath;
...
const slideio::NDPITiffDirectory& slideio::NDPIFile::findZoomDirectory(double zoom, int sceneWidth, int dirBegin,
                                                                       int dirEnd)
{
