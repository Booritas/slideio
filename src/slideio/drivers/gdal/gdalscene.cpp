...
slideio::GDALScene::GDALScene(SmallImagePage* page, const std::string& path, const std::string& driverId)
    : m_imagePage(page), m_filePath(path), m_driverId(driverId)
{
    m_filePath = path;
}
...
{
    if (m_imagePage == nullptr) RAISE_RUNTIME_ERROR << "GDALDriver: Invalid image header by channel number query";
    return m_imagePage->getNumChannels();
}

slideio::DataType slideio::GDALScene::getChannelDataType(int channel) const
{
    if (m_imagePage == nullptr) RAISE_RUNTIME_ERROR << "GDALDriver: Invalid image header by data type query";
...
{
    if (m_imagePage == nullptr) RAISE_RUNTIME_ERROR << "GDALDriver: Invalid image header by scene size query";
                                                      ...
                                                      int tFrameIndex, cv::OutputArray output)
{
    ...
    if (m_imagePage == nullptr) RAISE_RUNTIME_ERROR << "GDALDriver: Invalid file header by raster reading operation";
...
{
    if (m_imagePage == nullptr) RAISE_RUNTIME_ERROR << "GDALDriver: Invalid image header by compression query";
