...
{
    if (m_files.empty()) RAISE_RUNTIME_ERROR << "DICOM WSI: No files in the scene";
    return m_files[0];
}
                                            ...
                                            cv::OutputArray output)
{
    ...
    auto& files = m_files;
    userData.zoomLevelIndex = Tools::findZoomLevel(zoom, static_cast<int>(m_files.size()),
                                                   [&files](int index) { return files[index]->getScale(); });
...
std::shared_ptr<CVScene> WSIScene::getAuxImage(const std::string& imageName) const
{
    ...
    if (it == m_auxImages.end()) RAISE_RUNTIME_ERROR << "DICOM WSI: cannot find aux image " + imageName;
