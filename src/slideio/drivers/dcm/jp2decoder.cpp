...
bool JP2Decode(unsigned char* pJp2Stream, long nJp2StreamLength, DataType dt, Jp2Decoder::Photometric ePh, int nWidth,
               int nHeight, int nChnls, unsigned char* p_buff, long buff_size);
                               ...
                               const DcmStack& objStack, OFBool& removeOldRep) const
{
    ...
    else
    {
        ...
        if (result.good())
            result = OFreinterpret_cast(DcmItem*, dataset)->findAndGetUint16(DCM_SamplesPerPixel, imageSamplesPerPixel);
        ...
        if (imageFrames < 1) imageFrames = 1;
            ...
            if (result.good() && (pixItem != nullptr))
            {
                ...
                if (result.good())
                {
                    ...
                    else
                    {
                        ...
                        if (imageRows != 0 &&
                            frameSize / imageRows != (imageBytesAllocated * imageColumns * imageSamplesPerPixel))
                        {
                            DCMJPEG_WARN(
                        ...
                        if (totalSize == 0xFFFFFFFF ||
                            (frameSize != 0 && totalSize / frameSize != OFstatic_cast(Uint32, imageFrames)))
                        {
                            DCMJPEG_WARN(
                                "cannot decompress image because uncompressed representation would exceed maximum possible size of PixelData attribute");
                            return EC_ElemLengthExceeds32BitField;
...
OFCondition Jp2Decoder::encode(const E_TransferSyntax fromRepType, const DcmRepresentationParameter* fromRepParam,
                               DcmPixelSequence* fromPixSeq, const DcmRepresentationParameter* toRepParam,
                               DcmPixelSequence*& toPixSeq, const DcmCodecParameter* cp, DcmStack& objStack,
...
OFCondition Jp2Decoder::determineDecompressedColorModel(const DcmRepresentationParameter* fromParam,
                                                        ...
                                                        DcmItem* dataset, std::string& decompressedColorModel) const
{
    return EC_IllegalCall;
}
                               ...
                               const DcmStack& objStack) const
{
    ...
    else
    {
        ...
        if (result.good()) imageBytesAllocated = OFstatic_cast(Uint16, imageBitsAllocated / 8);
        ...
        if (result.good())
        {
            ...
            if (result.good())
            {
                ...
                if ((ePh == DVPI_Unknown) && (imageSamplesPerPixel > 1)) result = EC_CannotChangeRepresentation;
            }
        }
        ...
        if (imageFrames > 1) result = EC_CannotChangeRepresentation; // not yet supported
