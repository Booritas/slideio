    ...
    namespace vsi
    {
        ...
        public:
            TagInfo(TagInfo&& other) noexcept
                : tag(other.tag), fieldType(other.fieldType), valueType(other.valueType),
                  extendedType(other.extendedType), secondTag(other.secondTag), extended(other.extended),
                  dataSize(other.dataSize), name(std::move(other.name)), children(std::move(other.children)),
            ...
            TagInfo() = default;
            TagInfo(const TagInfo& other) { copy(other); }
            ...
            const_iterator find(int tag)
            {
                return std::find_if(children.begin(), children.end(),
                                    [tag](const TagInfo& info) { return info.tag == tag; });
            }
            void addChild(const TagInfo& info) { children.push_back(info); }
            void setValue(const std::string& srcValue) { this->value = srcValue; }
            ...
            const TagInfo* findChild(int srcTag) const
            {
                auto it = std::find_if(children.begin(), children.end(),
                                       [srcTag](const TagInfo& info) { return info.tag == srcTag; });
                if (it == children.end()) return nullptr;
                return &(*it);
            }
            const_iterator findNextChild(int srcTag, const_iterator begin) const
            {
                return std::find_if(begin, children.end(),
                                    [srcTag](const TagInfo& info) { return info.tag == srcTag; });
            }
            ...
            const vsi::TagInfo* findChildRecursively(const int tag) const
            {
                for (auto& child : children)
                {
                    ...
                    if (const TagInfo* found = child.findChildRecursively(tag)) return found;
                }
                return nullptr;
            }
