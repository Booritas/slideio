...
std::string VSIStream::readString(size_t dataSize)
{
    ...
    if (!Endian::isLittleEndian()) wstr = Endian::u16StringLittleToBig(wstr);
...
{
    m_stream->seekg(pos);
    if (m_stream->bad()) RAISE_RUNTIME_ERROR << "VSI driver: error by setting stream position: " << pos;
}
...
int64_t VSIStream::getSize()
{
    if (m_size < 0)
    {
        ...
        m_stream->seekg(pos);
        if (m_stream->bad()) RAISE_RUNTIME_ERROR << "VSI driver: error by getting stream size";
    }
    return m_size;
}
...
void VSIStream::skipBytes(uint32_t bytes)
{
    ...
    if (m_stream->bad()) RAISE_RUNTIME_ERROR << "VSI driver: error by skipping stream bytes";
}

void VSIStream::readBytes(uint8_t* buffer, uint32_t size)
{
    m_stream->read(reinterpret_cast<char*>(buffer), size);
    if (m_stream->bad()) RAISE_RUNTIME_ERROR << "VSI driver: error by reading " << size << " bytes from stream";
}
