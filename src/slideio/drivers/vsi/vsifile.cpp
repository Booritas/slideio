...
static bool parseRgbTripleFromValue(const std::string& s, uint8_t& r, uint8_t& g, uint8_t& b)
{
    ...
    int ri = 0, gi = 0, bi = 0;
    if (!extract("red", ri) || !extract("green", gi) || !extract("blue", bi)) return false;
    if (ri < 0) ri = 0;
    if (ri > 255) ri = 255;
    if (gi < 0) gi = 0;
    if (gi > 255) gi = 255;
...
{
    const std::vector<int> REL_PATH_TO_MAGNIFICATION = {Tag::MICROSCOPE, Tag::MICROSCOPE_PROPERTIES,
                                                        Tag::OPTICAL_PROPERTIES, Tag::OBJECTIVE_MAG};

    const std::vector<int> REL_PATH_TO_MAGNIFICATION2 = {Tag::MICROSCOPE, 4, Tag::OPTICAL_PROPERTIES,
                                                         Tag::OBJECTIVE_MAG};

    const std::vector<int> REL_PATH_TO_BITDEPTH = {Tag::MICROSCOPE, Tag::MICROSCOPE_PROPERTIES, Tag::OPTICAL_PROPERTIES,
                                                   Tag::BIT_DEPTH};
    ...
    if (!m_metadata.empty())
    {
        ...
        for (const TagInfo* const& volume : volumes)
        {
            ...
            if (frames.size() > 1) RAISE_RUNTIME_ERROR << "VSI Unexpected stack with multiple external files";
            if (!frames.empty())
            {
                ...
                if (ifdTag) volumeObj->setIFD(ifdTag->secondTag);
                if (color)
                {
                    try
                    {
                ...
                if (stackProps)
                {
                    ...
                    if (stackName) volumeObj->setName(stackName->value);
                    ...
                    if (!magnification) magnification = stackProps->findChild(REL_PATH_TO_MAGNIFICATION2);
                    if (!magnification)
                    {
                        if (microscope) magnification = microscope->findChildRecursively(Tag::OBJECTIVE_MAG);
                    }
                    if (magnification)
                    {
                        try
                ...
                for (TagInfo::const_iterator it = volume->begin(); it != volume->end(); ++it)
                {
                    ...
                    if (it == volume->end()) break;
                    const TagInfo& dimensionDescription = *it;
                    const int index = dimensionDescription.secondTag;
                    for (auto itc = dimensionDescription.children.begin(); itc != dimensionDescription.children.end();
                         ++itc)
                    {
                        if (itc->tag == Tag::DIMENSION_PARAMETERS)
                        {
                            ...
                            if (itc->secondTag >= 0)
                            {
                                ...
                                if (channelName) volumeObj->setChannelName(channelIndex, channelName->value);
                                ...
                                if (const TagInfo* wavelength = itc->findChild(Tag::CHANNEL_WAVELENGTH))
                                {
                                    if (const TagInfo* wlValue = wavelength->findChild(Tag::VALUE))
                                    {
                                        try
                                        {
                                            ...
                                            if (nm > 0.0) volumeObj->setChannelEmissionWavelength(channelIndex, nm);
                                        }
                                        catch (const std::exception&)
                                        {
...
void VSIFile::assignAuxVolumes()
{
    try
    {
        ...
        for (std::shared_ptr<Volume>& volume : m_volumes)
        {
            ...
            case StackType::EFI_STACK:
            case StackType::MACRO_IMAGE:
                if (lastImageVolume) lastImageVolume->addAuxVolume(volume);
                break;
            }
        }
    }
...
void VSIFile::getVolumeMetadataItems(std::list<const TagInfo*>& volumes) const
{
    ...
    if (volumeCollection)
    {
        for (auto itPos = volumeCollection->begin(); itPos != volumeCollection->end(); ++itPos)
        {
            ...
            if (itVolume == volumeCollection->end()) break;
...
void VSIFile::getImageFrameMetadataItems(const TagInfo* volume, std::list<const TagInfo*>& frames)
{
    for (auto itPos = volume->begin(); itPos != volume->end(); ++itPos)
    {
        ...
        if (itFrame == volume->end()) break;
        if (itFrame->findChild(Tag::EXTERNAL_FILE_PROPERTIES) || itFrame->findChild(Tag::DEFAULT_SAMPLE_PIXEL_DATA_IFD))
        {
...
{
    const auto& it = std::remove_if(m_volumes.begin(), m_volumes.end(),
                                    [](const std::shared_ptr<Volume>& vol) { return !vol->isValid(); });
    m_volumes.erase(it, m_volumes.end());
}
...
void VSIFile::read()
{
    ...
    assignAuxVolumes();
    cleanVolumes();
    if (m_expectExternalFiles) readExternalFiles();
}
...
void VSIFile::checkExternalFilePresence()
{
    ...
    for (auto& volume : volumes)
    {
        ...
        if (m_expectExternalFiles) break;
    }
...
void VSIFile::readVolumeInfo()
{
    ...
    if (strncmp(reinterpret_cast<char*>(header.magic), "II", 2) != 0)
        RAISE_RUNTIME_ERROR << "VSI driver: invalid file header. Expected first two bytes: 'II', got: " << header.magic;
    if (header.i42 != 42)
        RAISE_RUNTIME_ERROR << "VSI driver: invalid file header. Expected second word: 42, got: " << header.i42;
    std::list<TagInfo> path = {m_metadata};
    readMetadata(vsiStream, path);
    if (path.size() != 1) RAISE_RUNTIME_ERROR << "VSI driver: invalid metadata path";
                                    ...
                                    std::list<std::pair<EtsFilePtr, TileInfoListPtr>> orphanEtsFiles)
{
    ...
    if (!orphanEtsFiles.empty())
        SLIDEIO_LOG(WARNING) << "VSI driver: " << orphanEtsFiles.size() << "ETS files cannot be assigned to any volume";
}
...
void VSIFile::readExternalFiles()
{
    ...
    if (fs::exists(subDirPath))
    {
        ...
        if (!orphanEtsFiles.empty() && !volumes.empty()) processOrphanEtsFiles(volumes, orphanEtsFiles);
    }
}
...
bool VSIFile::readVolumeHeader(VSIStream& vsi, VolumeHeader& volumeHeader)
{
    ...
    if (volumeHeader.headerSize != 24) return false;
    if (volumeHeader.magicNumber != 21321) return false;
    if (volumeHeader.offsetFirstDataField < 0) return false;
    return true;
}
...
bool VSIFile::readMetadata(VSIStream& vsi, std::list<TagInfo>& path)
{
    ...
    if (!readVolumeHeader(vsi, volumeHeader)) return false;
    ...
    if (dataFieldOffset >= vsi.getSize()) return false;
    ...
    for (uint tagIndex = 0; tagIndex < childCount; ++tagIndex)
    {
        ...
        if (tagHeader.tag < 0)
        {
            if (!inlineData && (tagHeader.dataSize + vsi.getPos()) < vsi.getSize()) vsi.skipBytes(tagHeader.dataSize);
            return false;
        }
        ...
        else
        {
            ...
            if (!inlineData && tagInfo.dataSize > 0) value = VSITools::extractTagValue(vsi, tagInfo);
