...
void slideio::vsi::EtsFile::read(std::list<std::shared_ptr<Volume>> &volumes)
{
   ...
   for (auto it = volumes.begin(); it != volumes.end(); ++it)
   {
      ...
      if (volumeWidth >= minWidth && volumeWidth <= maxWidth && volumeHeight >= minHeight && volumeHeight <= maxHeight)
      {
         volumes.erase(it);
         assignVolume(volume);
         break;
   ...
   processPyramid(numPyramidLevels, tiles);
}

void slideio::vsi::EtsFile::processPyramid(int numPyramidLevels, std::vector<TileInfo> &tiles) {
   m_pyramid.resize(numPyramidLevels);
   for (int level = 0; level < numPyramidLevels; ++level) {
      PyramidLevel &pyramidLevel = m_pyramid[level];
      const int width = m_sizeWithCompleteTiles.width >> level;
      const int height = m_sizeWithCompleteTiles.height >> level;
   ...
   if(channelIndex>0)
    ...
    for(auto& pyramidLevel: m_pyramid) {
