...
EtsFileScene::EtsFileScene(const std::string& filePath, int sceneIndex, const std::string& driverId,
                           std::shared_ptr<vsi::VSIFile>& vsiFile, int etsIndex)
    : VSIScene(filePath, sceneIndex, driverId, vsiFile), m_etsIndex(etsIndex)
{
    init();
}
...
int EtsFileScene::getTileCount(void* userData)
{
    ...
    if (!etsFile) RAISE_RUNTIME_ERROR << "VSIImageDriver: ETS file is not initialized";
...
bool EtsFileScene::getTileRect(int tileIndex, cv::Rect& tileRect, void* userData)
{
    ...
    {
        RAISE_RUNTIME_ERROR << "VSIImageDriver: Tile index " << tileIndex << " is out of range (0 - "
                            << level.getNumTiles() << ")";
    }
...
{
    if (m_auxScenes.count(imageName)) return m_auxScenes.at(imageName);
...
{
    if (getEtsFile() && getEtsFile()->getVolume()) return getEtsFile()->getVolume()->getZResolution();
    return 0.;
}

double EtsFileScene::getTFrameResolution() const
{
    if (getEtsFile() && getEtsFile()->getVolume()) return getEtsFile()->getVolume()->getTResolution();
    return 0.;
}

int EtsFileScene::getNumChannels() const
{
    if (getEtsFile()) return getEtsFile()->getNumChannels();
    return 0;
}
...
std::string EtsFileScene::getChannelName(int channel) const
{
    ...
    if (!volume) RAISE_RUNTIME_ERROR << "VSIImageDriver: ETS file does not contain volume";
                                                ...
                                                int tFrameIndex, cv::OutputArray output)
{
    ...
    if (levelIndex < 0 || levelIndex >= etsFile->getNumPyramidLevels())
    {
        RAISE_RUNTIME_ERROR << "VSIImageDriver: Unexpected zoom level index: " << levelIndex << " Expected: " << "0 - "
                            << etsFile->getNumPyramidLevels();
    }
    const PyramidLevel& level = etsFile->getPyramidLevel(levelIndex);
...
void EtsFileScene::init()
{
    ...
    if (volume)
    {
        ...
        for (int ch = 0; ch < m_numChannels; ++ch)
        {
            if (volume->hasChannelColor(ch))
            {
                ...
                char buf[8];
                std::snprintf(buf, sizeof(buf), "#%02X%02X%02X", static_cast<unsigned int>(r),
                              static_cast<unsigned int>(g), static_cast<unsigned int>(b));
                setChannelAttribute(ch, "Color", std::string(buf));
            }
            const double nm = volume->getChannelEmissionWavelength(ch);
            if (nm > 0.0) setChannelAttribute(ch, "EmissionWavelength", nm);
        }
    }
    ...
    for (int levelIndex = 0; levelIndex < etsFile->getNumPyramidLevels(); ++levelIndex)
    {
        ...
        const double scale =
            static_cast<double>(pyramidLevel.getSize().width) / static_cast<double>(etsFile->getSize().width);
...
std::shared_ptr<vsi::EtsFile> EtsFileScene::getEtsFile() const
{
    ...
    {
        RAISE_RUNTIME_ERROR << "VSIImageDriver: ETS index " << m_etsIndex << " is out of range (0-"
                            << m_vsiFile->getNumEtsFiles() << ")";
    }
