...
PKETiledScene::PKETiledScene(const std::string& filePath, int sceneIndex, const std::string& driverId,
                             const std::string& name, const std::vector<TiffDirectory>& dirs)
    : PKEScene(filePath, sceneIndex, driverId, name), m_directories(dirs)
{
    initialize();
}
...
PKETiledScene::PKETiledScene(const std::string& filePath, int sceneIndex, const std::string& driverId,
    ...
    : PKEScene(filePath, sceneIndex, driverId, hFile, name), m_directories(dirs)
{
    initialize();
}
...
void PKETiledScene::initialize()
{
    ...
    if (!description.empty())
    {
        ...
        if (!root) RAISE_RUNTIME_ERROR << "PKEImageDriver: Error parsing image description xml: root element is null";
        ...
        if (xmlScanResolution)
        {
            ...
            if (xmlMagnification) magnification = xmlMagnification->DoubleText();
        }
        if (magnification < 0)
        {
            auto xmlObjective = root->FirstChildElement("Objective");
            if (xmlObjective) magnification = xmlObjective->DoubleText();
        }
        if (magnification > 0) m_magnification = magnification;
        // Extract name
        auto xmlName = root->FirstChildElement("SlideID");
        if (xmlName) m_name = xmlName->GetText();
        // Extract isUnmixed
        auto xmlUnmixed = root->FirstChildElement("IsUnmixedComponent");
        if (xmlUnmixed) m_isUnmixed = xmlUnmixed->BoolText();
    }
    ...
    if (!m_directories.empty())
    {
        ...
        {
            if (dir.width == fullResolutionWidth && dir.height == fullResolutionHeight) numFullResolutions++;
...
void PKETiledScene::initializeChannelNames()
{
    ...
    if (dir0.channels == 1)
    {
        for (int channel = 0; channel < m_numChannels; ++channel)
        {
            ...
            if (xmlName) name = xmlName->GetText();
            ...
            if (xmlColor)
            {
                ...
                if (colorText != nullptr)
                {
                    try
                    {
                        setChannelAttribute(channel, "Color", ColorTools::rgbCsvToHexRGB(colorText));
                    }
...
{
    if (zSliceIndex != 0 || tFrameIndex != 0) RAISE_RUNTIME_ERROR << "PKEDriver: 3D and 4D images are not supported";
    auto hFile = getFileHandle();
    if (hFile == nullptr) throw std::runtime_error("PKEDriver: Invalid file header by raster reading operation");
...
bool slideio::PKETiledScene::readTiffTile(int tileIndex, int zoomLevel, const std::vector<int>& channelIndices,
                                          cv::OutputArray tileRaster)
{
    ...
    try
    {
        ...
        else if (dir.channels == 1)
        {
            ...
            {
                cv::Mat channelRaster;
                TiffTools::readTile(getFileHandle(), m_directories[dirIndex + channelIndex], tileIndex, {0},
                                    channelRaster);
                channelRasters.push_back(channelRaster);
            }
            cv::merge(channelRasters, tileRaster);
                             ...
                             void* userData)
{
    ...
    const int dirIndex = m_zoomDirectoryIndices[level];
    const TiffDirectory& dir = m_directories[dirIndex];
    if (dir.tiled) return readTiffTile(tileIndex, level, channelIndices, tileRaster);
    if (dir.channels == getNumChannels()) return readTiffDirectory(dir, channelIndices, tileRaster);
    if (dir.channels == 1)
    {
    ...
    {
        RAISE_RUNTIME_ERROR << "PerkinElmer driver: Unexpected number of channels in the directory: " << dir.channels
