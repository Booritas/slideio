    ...
    namespace converter
    {
        ...
        public:
            ...
            TiffConverter(const TiffConverter& other)
                : m_pages(other.m_pages), m_file(other.m_file), m_scene(other.m_scene),
                  m_parameters(other.m_parameters), m_cropRect(other.m_cropRect), m_filePath(other.m_filePath),
                  m_totalTiles(other.m_totalTiles), m_currentTile(other.m_currentTile),
                  m_lastProgress(other.m_lastProgress), m_readersIdleTimeNs(other.m_readersIdleTimeNs.load()),
                  m_encodersIdleTimeNs(other.m_encodersIdleTimeNs.load()),
                  m_writerIdleTimeNs(other.m_writerIdleTimeNs.load()), m_numReaderThreads(other.m_numReaderThreads),
        ...
        public:
            ...
            int getNumTiffPages() const { return static_cast<int>(m_pages.size()); }

            const TiffPageStructure& getTiffPage(int index) const;
            Rect getSceneRect() const { return m_cropRect; }

            int getTotalTiles() const { return m_totalTiles; }

            const ConverterParameters& getParameters() const { return m_parameters; }
            ...
            int getNumReaderThreads() const { return m_numReaderThreads; }
            int getNumEncoderThreads() const { return m_numEncoderThreads; }
            int getNumWriterThreads() const { return 1; }
            void createTileQueue(const TiffDirectory& dir, const TiffDirectoryStructure& page, int tileBatchSize,
                                 std::queue<Block>& queue);
            virtual std::pair<std::shared_ptr<Slide>, std::shared_ptr<Scene>> cloneScene() const;

        protected:
            std::shared_ptr<CVScene> getScene() const { return m_scene; }

        private:
            TiffPageStructure& appendPage() { return m_pages.emplace_back(); }
            ...
            void readTiles(const TiffDirectory& dir, const TiffDirectoryStructure& page, BoundedQueue<Tile>& inputQueue,
                           std::queue<Block>& blockQueue, std::mutex& blockQueueMutex,
                           std::atomic<size_t>& activeReaders, std::exception_ptr& readerException,
            ...
            void writeTiles(BoundedQueue<Tile>& inputQueue, BoundedQueue<EncodedTile>& outputQueue,
