...
void ConverterParameters::copyFrom(const ConverterParameters& other)
{
    ...
    if (other.m_encodeParameters)
    {
        ...
        else if (compression == Compression::Jpeg2000)
        {
            ...
            auto newParams =
                std::make_shared<JP2KEncodeParameters>(jp2kParams->getCompressionRate(), jp2kParams->getCodecFormat());
...
void ConverterParameters::updateNotDefinedParameters(const std::shared_ptr<CVScene>& scene)
{
    ...
    if (m_channelRange.size() <= 0) m_channelRange = Range(0, scene->getNumChannels());
...
{
    if (m_encodeParameters == nullptr) RAISE_RUNTIME_ERROR << "Converter: Image encoding parameters are not defined!";
