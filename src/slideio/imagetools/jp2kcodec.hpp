...
public:
    ...
    ~wopj_cparameters()
    {
        if (cp_comment) free(cp_comment);
        if (cp_matrice) free(cp_matrice);
    }
};
...
public:
    ...
    CodecPtr& operator=(const CodecPtr& other)
    {
        if (this != &other)
        {
            if (m_codec) opj_destroy_codec(m_codec);
            m_codec = other.m_codec;
            const_cast<CodecPtr&>(other).m_codec = nullptr;
        }
        return *this;
    ...
    opj_codec_t* get() const { return m_codec; }

    operator opj_codec_t*() const { return m_codec; }
...
public:
    ...
    ImagePtr& operator=(const ImagePtr& other)
    {
        if (this != &other)
        {
            if (m_image) opj_image_destroy(m_image);
            m_image = other.m_image;
            const_cast<ImagePtr&>(other).m_image = nullptr;
        }
        return *this;
    ...
    opj_image_t* get() const { return m_image; }

    operator opj_image_t*() const { return m_image; }
...
public:
    ...
    StreamPtr& operator=(const StreamPtr& other)
    {
        if (this != &other)
        {
            if (m_stream) opj_stream_destroy(m_stream);
            m_stream = other.m_stream;
            const_cast<StreamPtr&>(other).m_stream = nullptr;
        }
        return *this;
    ...
    opj_stream_t* get() const { return m_stream; }

    operator opj_stream_t*() const { return m_stream; }
