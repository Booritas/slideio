...
namespace
{
    void addTag(json& root, FITAG* tag)
    {
        ...
        if (desc && desc[0] != '\0') tagObj["description"] = desc;
...
FIWrapper::Page::Page(FIWrapper* parent, int pageIndex)
{
    ...
    if (!m_pBitmap) RAISE_RUNTIME_ERROR << "FIWrapper::FIPage: Failed to lock page " << pageIndex;
    preparePage();
}
...
{
    if (m_fiFormat != FIF_TIFF) RAISE_RUNTIME_ERROR << "FIWrapper::Page::extractTiffCompression: Not a TIFF format";
...
void FIWrapper::Page::detectNumChannels()
{
    ...
    else if (imageType == FIT_UINT16 || imageType == FIT_INT16 || imageType == FIT_UINT32 || imageType == FIT_INT32 ||
...
{
    if (!m_pBitmap) RAISE_RUNTIME_ERROR << "FIWrapper::Page::readRaster: Invalid bitmap";
    ...
    if (!bits) RAISE_RUNTIME_ERROR << "FIWrapper::Page::readRaster: Cannot get bitmap bits";
...
Resolution FIWrapper::Page::getResolution() const
{
    ...
    if (pixPerMX != 0) metersPerPixelX = 1.0 / pixPerMX;
    if (pixPerMY != 0) metersPerPixelY = 1.0 / pixPerMY;
...
void FIWrapper::Page::extractTiffMetadata()
{
    if (m_fiFormat == FIF_TIFF)
    {
        ...
        if (!dir.description.empty()) mtdObj["Description"] = dir.description;
        if (!dir.software.empty()) mtdObj["Software"] = dir.software;
...
void FIWrapper::Page::extractCommonMetadata()
{
    ...
    static constexpr FREE_IMAGE_MDMODEL models[] = {
        FIMD_EXIF_MAIN, FIMD_EXIF_EXIF, FIMD_EXIF_GPS, FIMD_EXIF_MAKERNOTE, FIMD_EXIF_INTEROP, FIMD_IPTC,
        FIMD_XMP,       FIMD_ANIMATION, FIMD_COMMENTS, FIMD_CUSTOM,         FIMD_GEOTIFF};
...
FIWrapper::FIWrapper(const std::string& filePath)
{
    ...
    if (!std::filesystem::exists(filePath)) forceUnicode = std::filesystem::exists(wsPath);
#endif

#if defined(WIN32)
    if (!std::filesystem::exists(wsPath)) RAISE_RUNTIME_ERROR << "FIWrapper: File " << filePath << " does not exist";
    m_fiFormat = FreeImage_GetFileTypeU(wsPath.c_str(), 0);
    if (m_fiFormat == FIF_UNKNOWN) m_fiFormat = FreeImage_GetFIFFromFilenameU(wsPath.c_str());
#else
    if (!std::filesystem::exists(filePath)) RAISE_RUNTIME_ERROR << "FIWrapper: File " << filePath << " does not exist";
    m_fiFormat = FreeImage_GetFileType(filePath.c_str(), 0);
    if (m_fiFormat == FIF_UNKNOWN) m_fiFormat = FreeImage_GetFIFFromFilename(filePath.c_str());
#endif
    ...
    if (forceUnicode)
    {
        RAISE_RUNTIME_ERROR << "FIWrapper: Unexpected error: Cannot force unicode path on non-Windows platform: "
                            << filePath;
    }
#endif

    if (m_fiFormat == FIF_TIFF) TiffTools::scanFile(filePath, m_tiffDirectories);
    ...
    if (!forceUnicode && (m_fiFormat == FIF_TIFF || m_fiFormat == FIF_GIF || m_fiFormat == FIF_ICO))
    {
        m_hFile = FreeImage_OpenMultiBitmap(m_fiFormat, filePath.c_str(),
                                            ...
                                            0);    // flags
        if (!m_hFile) RAISE_RUNTIME_ERROR << "FIWrapper: Failed to open multipage image";
    }
    else
    {
#if defined(WIN32)
        ...
        m_pages[0] = pagePtr;
        if (!m_pBitmap) RAISE_RUNTIME_ERROR << "FIWrapper: Failed to load image";
    }
}
...
{
    if (m_hFile) return FreeImage_GetPageCount(m_hFile);
...
SmallImagePage* FIWrapper::readPage(int page)
{
    if (page >= getNumPages())
    {
        RAISE_RUNTIME_ERROR << "FIWrapper::getPage: Page index out of range: " << page
                            << ". Number of pages: " << getNumPages();
    }
    auto it = m_pages.find(page);
    if (it != m_pages.end()) return it->second.get();
    if (m_hFile != nullptr)
    {
...
const TiffDirectory& FIWrapper::getTiffDirectory(int index) const
{
    ...
    {
        RAISE_RUNTIME_ERROR << "FIWrapper::getTiffDirectory: Index out of range: " << index
                            << ". Number of TIFF directories: " << m_tiffDirectories.size();
    }
...
{
    if (raster.empty()) RAISE_RUNTIME_ERROR << "FIWrapper::writeRaster: Input raster is empty";

    FREE_IMAGE_FORMAT fif = FreeImage_GetFIFFromFilename(filePath.c_str());

    if (fif == FIF_UNKNOWN) fif = mapCompressionToImageFormat(compression);

    if (fif == FIF_UNKNOWN || !FreeImage_FIFSupportsWriting(fif))
    {
        RAISE_RUNTIME_ERROR << "FIWrapper::writeRaster: Cannot detect file format for file: " << filePath
    ...
    {
        RAISE_RUNTIME_ERROR << "FIWrapper::writeRaster: Cannot define image type from raster. Channels: " << channels
                            << ", Depth: " << depth;
    }
    ...
    if (!bitmap) RAISE_RUNTIME_ERROR << "FIWrapper::writeRaster: Failed to allocate FreeImage bitmap";
    ...
    try
    {
        ...
        if (!success) RAISE_RUNTIME_ERROR << "FIWrapper::writeRaster: Failed to save image to " << filePath;
