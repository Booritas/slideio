...
void SmallImage::readImageStack(cv::OutputArray raster)
{
    ...
    for (int i = 0; i < numPages; ++i)
    {
        ...
        if (!page) RAISE_RUNTIME_ERROR << "SmallImage::readImageStack: invalid image page encountered.";
    ...
    if (!pageRasters.empty()) cv::merge(pageRasters, raster);
}
...
void slideio::ImageTools::readSmallImageRaster(const std::string& filePath, cv::OutputArray output)
{
    ...
    if (!image->isValid()) RAISE_RUNTIME_ERROR << "Cannot open file: " << filePath;
