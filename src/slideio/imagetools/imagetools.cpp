...
namespace
{
    bool isTiff(const std::string& filePath)
    {
        ...
        if (fiFormat == FIF_UNKNOWN) fiFormat = FreeImage_GetFIFFromFilenameU(wsPath.c_str());
#else
        if (!std::filesystem::exists(filePath))
            RAISE_RUNTIME_ERROR << "FIWrapper: File " << filePath << " does not exist";
        fiFormat = FreeImage_GetFileType(filePath.c_str(), 0);
        if (fiFormat == FIF_UNKNOWN) fiFormat = FreeImage_GetFIFFromFilename(filePath.c_str());
#endif
...
double ImageTools::computeSimilarity(const cv::Mat& leftM, const cv::Mat& rightM, bool ignoreTypes)
{
    ...
    {
        RAISE_RUNTIME_ERROR << "Image sizes for comparison shall be equal. Left image: (" << leftSize.width << ","
                            << leftSize.height << "), Right image: (" << rightSize.width << "," << rightSize.height
                            << ")";
    }
    if (leftM.channels() != rightM.channels())
    {
        RAISE_RUNTIME_ERROR << "Number of image channesl for comparison shall be equal. Left image: "
                            << leftM.channels() << ". Right image:" << rightM.channels() << ".";
    }
...
double ImageTools::computeSimilarity2(const cv::Mat& leftM, const cv::Mat& rightM)
{
    ...
    double maxVal = 255;
    {
        ...
        std::vector<double> minLeft(channelCount), maxLeft(channelCount), minRight(channelCount),
            maxRight(channelCount);
    ...
    if (std::fabs(maxVal) > 0.) diffd /= maxVal;
...
double ImageTools::compareHistograms(const cv::Mat& left, const cv::Mat& right, int binCount)
{
    ...
    cv::calcHist(&left, 1, channels.data(), mask, histLeft, channelCount, histSizes.data(),
                 (const float**)ranges.data());

    cv::calcHist(&right, 1, channels.data(), mask, histRight, channelCount, histSizes.data(),
                 (const float**)ranges.data());
...
void ImageTools::readBitmap(const std::string& path, cv::OutputArray output)
{
    ...
    if (!file.is_open()) RAISE_RUNTIME_ERROR << "ImageTools::readBitmap: Cannot open file " << path;
    ...
    SLIDEIO_LOG(INFO) << "ImageTools::readBitmap: Successfully read BMP file " << path << " (" << width << "x" << height
                      << ", " << bitCount << " bits)";
}
