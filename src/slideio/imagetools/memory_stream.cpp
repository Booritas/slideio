...
{
    if (!data || !data->data || data->size == 0 || data->offset >= data->size) return -1;
...
OPJ_OFF_T skipMemory(OPJ_OFF_T bytes, OPJStreamUserData* data)
{
    if (!data || !data->data || data->size == 0 || bytes < 0) return -1;
...
OPJ_BOOL seekMemory(OPJ_OFF_T bytes, OPJStreamUserData* data)
{
    if (!data || !data->data || data->size == 0 || bytes < 0) return OPJ_FALSE;
