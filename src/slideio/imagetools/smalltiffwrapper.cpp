...
SmallImagePage* slideio::SmallTiffWrapper::readPage(int pageIndex)
{
    ...
    {
        RAISE_RUNTIME_ERROR << "SmallTiffWrapper: invalid page index: " << pageIndex << ". Valid range is [0, "
                            << getNumPages() - 1 << "]";
    }
    if (m_pages.size() != m_directories.size()) m_pages.resize(m_directories.size());

    if (!m_pages[pageIndex]) m_pages[pageIndex] = std::make_shared<SmallTiffPage>(this, pageIndex);
