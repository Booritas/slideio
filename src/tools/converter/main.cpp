...
{
    if (rectStr.empty()) return false;
    std::istringstream iss(rectStr);
    char c1, c2, c3;
    if (!(iss >> rect.x >> c1 >> rect.y >> c2 >> rect.width >> c3 >> rect.height) || c1 != ',' || c2 != ',' ||
        c3 != ',')
    {
        throw std::runtime_error("Invalid rectangle format. Use: x,y,width,height");
    }
    if (rect.width <= 0 || rect.height <= 0) throw std::runtime_error("Rectangle width and height must be positive");
    if (rect.x < 0 || rect.y < 0) throw std::runtime_error("Rectangle x and y must be non-negative");
    return true;
}

static bool parseRange(const std::string& rangeStr, Range& range)
{
    if (rangeStr.empty()) return false;
    std::istringstream iss(rangeStr);
    char c;
    if (!(iss >> range.start >> c >> range.end) || c != ',')
        throw std::runtime_error("Invalid range format. Use: start,end");
    if (range.start < 0 || range.end < 0) throw std::runtime_error("Range start and end must be non-negative");
    if (range.start >= range.end) throw std::runtime_error("Range start must be less than end");
    return true;
}
...
int main(int argc, char* argv[])
{
    ...
    app.add_option("input", inputPath, "Input file path")->required()->check(CLI::ExistingFile);

    app.add_option("output", outputPath, "Output file path")->required();
    ...
    app.add_option("-d,--driver", inputDriver, "Input driver name (AUTO for auto-detection)")->default_val("AUTO");
    ...
    app.add_option("--slice-range", sliceRangeStr, "Z-slice range: start,end (empty for all slices)")->default_str("");

    app.add_option("--frame-range", frameRangeStr, "Time frame range: start,end (empty for all frames)")
        ->default_str("");

    app.add_flag("-s,--silent", silent, "Silent execution (no information, no progress bar)")->default_val(false);
    ...
    try
    {
        ...
        convertFile(inputPath, outputPath, sceneIndex, compressionRate, tileSize, numZoomLevels, inputDriver,
                    targetFormat, targetCompression, compressionQuality, rect, channelRange, sliceRange, frameRange,
                    silent, infoOnly, deleteIfExists, tileBatchSize, numReadingThreads, numEncodingThreads);
    }
