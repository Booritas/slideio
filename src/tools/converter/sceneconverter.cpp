...
void printInfo(const TiffConverter& converter)
{
    ...
    std::cout << "Target format: " << ((converter.getParameters().getFormat() == SVS) ? "SVS" : "OMETIFF") << std::endl;
    ...
    std::cout << "Reading threads: " << numReadingThreads
              << (numReadingThreads == 0 ? " (auto: half of CPU cores)" : "") << std::endl;
    std::cout << "Encoding threads: " << numEncodingThreads
              << (numEncodingThreads == 0 ? " (auto: half of CPU cores)" : "") << std::endl;
}

void convertFile(const std::string& inputPath, const std::string& outputPath, int sceneIndex, double compressionRate,
                 int tileSize, int numZoomLevels, const std::string& inputDriver, const std::string& targetFormat,
                 const std::string& targetCompression, int compressionQuality, const Rect& rect,
                 const Range& channelRange, const Range& sliceRange, const Range& frameRange, bool silent,
                 bool infoOnly, bool deleteIfExists, int tileBatchSize, int numReadingThreads, int numEncodingThreads)
{
    if (!std::filesystem::exists(inputPath)) throw std::runtime_error("Input file does not exist: " + inputPath);
    ...
    if (!rect.empty())
    {
        ...
        if (rect.x + rect.width > sceneRect.width || rect.y + rect.height > sceneRect.height)
        {
            RAISE_RUNTIME_ERROR << "Specified rectangle exceeds scene dimensions: (0,0," << sceneRect.width << ","
                                << sceneRect.height << ").";
        }
        params.setRect(rect);
    }
    if (!channelRange.empty())
    {
        ...
        if (channelRange.end > numChannels)
            RAISE_RUNTIME_ERROR << "Channel range exceeds number of channels in the scene: " << numChannels;
    ...
    if (!sliceRange.empty())
    {
        ...
        if (sliceRange.end > numSlices)
            RAISE_RUNTIME_ERROR << "Slice range exceeds number of slices in the scene: " << numSlices;
    ...
    if (!frameRange.empty())
    {
        ...
        if (frameRange.end > numFrames)
            RAISE_RUNTIME_ERROR << "Frame range exceeds number of frames in the scene: " << numFrames;
        params.setTFrameRange(frameRange);
    }
    if (compression == Compression::Jpeg2000)
    {
        auto jp2kParams = std::static_pointer_cast<JP2KEncodeParameters>(params.getEncodeParameters());
    ...
    if (numZoomLevels > 0) containerParams->setNumZoomLevels(numZoomLevels);
    TiffConverter converter;
    converter.createFileLayout(scene->getCVScene(), params);

    if (infoOnly || !silent) printInfo(converter);
    ...
    if (!silent)
    {
        ...
        std::cout << "Readers idle time: " << formatDuration(readersIdle) << " (" << converter.getNumReaderThreads()
                  << " threads)" << std::endl;
        std::cout << "Encoders idle time: " << formatDuration(encodersIdle) << " (" << converter.getNumEncoderThreads()
                  << " threads)" << std::endl;
        std::cout << "Writer idle time: " << formatDuration(writerIdle) << " (" << converter.getNumWriterThreads()
                  << " thread)" << std::endl;
    }
}
