...
void extractTiffData(const std::string& filePath, TIFFFiles& files, const std::string& imageId,
                     const std::string& attribute, int value, tinyxml2::XMLDocument& doc,
                     tinyxml2::XMLElement*& xmlTiffData)
{
    ...
    for (tinyxml2::XMLElement* xmlImage = doc.RootElement()->FirstChildElement("Image"); xmlImage != nullptr;
         xmlImage = xmlImage->NextSiblingElement("Image"))
    {
        ...
        if (imageId == currentImageId)
        {
            ...
            for (tinyxml2::XMLElement* xmlCurrentTiffData = xmlPixels->FirstChildElement("TiffData");
                 xmlCurrentTiffData != nullptr; xmlCurrentTiffData = xmlCurrentTiffData->NextSiblingElement("TiffData"))
            {
...
TEST_F(TiffDataTests, readTileChannelsTiledPalette3ch)
{
    ...
    std::string testFilePath = TestTools::getFullTestImagePath(
        "ometiff", "Tests/Leica-2.ome.tiff - Series 1 (1, x=23552, y=14336, w=512, h=512).png");
    std::string directoryPath = std::filesystem::path(filePath).parent_path().string();
    TIFFFiles files;
    tinyxml2::XMLElement* xmlTiffData = nullptr;
    tinyxml2::XMLDocument doc;
