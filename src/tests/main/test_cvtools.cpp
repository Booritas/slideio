...
TEST(CVTools, extractSliceFrom3D)
{
    ...
    {
        std::vector<cv::Range> ranges = {cv::Range(0, height), cv::Range(0, width), cv::Range(slice, slice + 1)};
...
TEST(CVTools, insertSliceInMultidimMatrix)
{
    ...
    std::vector<cv::Range> ranges = {cv::Range(0, height), cv::Range(0, width), cv::Range(3, 4)};
