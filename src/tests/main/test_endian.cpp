...
TEST_F(EndianTest, FromLittleEndianToNative_DataType_Byte)
{
    ...
    }
    {
        ...
        float expected[] = {Endian::isLittleEndian() ? 1234.5678f : Endian::swapBytes(data[0]),
                            Endian::isLittleEndian() ? 8765.4321f : Endian::swapBytes(data[1])};
    ...
    }
    {
        ...
        double expected[] = {Endian::isLittleEndian() ? 12345678.12345678 : Endian::swapBytes(data[0]),
                             Endian::isLittleEndian() ? 87654321.87654321 : Endian::swapBytes(data[1])};
