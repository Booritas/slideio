...
TEST(Tools, isXml) {
    // Minimal well-formed documents
    EXPECT_TRUE(Tools::isXml("<root/>"));
    EXPECT_TRUE(Tools::isXml("<root></root>"));
    EXPECT_TRUE(Tools::isXml("<root>text</root>"));

    // With an XML declaration
    EXPECT_TRUE(Tools::isXml("<?xml version=\"1.0\" encoding=\"UTF-8\"?><root/>"));

    // Nested elements, attributes and namespaces
    EXPECT_TRUE(Tools::isXml(
        "<?xml version=\"1.0\"?>\n"
        "<Image xmlns=\"http://www.openmicroscopy.org/Schemas/OME/2016-06\" ID=\"Image:0\">\n"
        "  <Pixels SizeX=\"100\" SizeY=\"200\" Type=\"uint8\">\n"
        "    <Channel ID=\"Channel:0\" Name=\"DAPI\"/>\n"
        "  </Pixels>\n"
        "</Image>\n"));

    // Leading whitespace and a UTF-8 BOM are allowed before the root element
    EXPECT_TRUE(Tools::isXml("   \r\n\t<root/>"));
    EXPECT_TRUE(Tools::isXml("\xEF\xBB\xBF<root/>"));
    EXPECT_TRUE(Tools::isXml("\xEF\xBB\xBF\n<?xml version=\"1.0\"?>\n<root/>"));

    // Comments and processing instructions around the root element
    EXPECT_TRUE(Tools::isXml("<!-- comment --><root/>"));
    EXPECT_TRUE(Tools::isXml("<root><!-- comment --></root>"));

    // CDATA and entities
    EXPECT_TRUE(Tools::isXml("<root><![CDATA[<not an element>]]></root>"));
    EXPECT_TRUE(Tools::isXml("<root>a &amp; b &lt; c</root>"));

    // Empty and whitespace-only input
    EXPECT_FALSE(Tools::isXml(""));
    EXPECT_FALSE(Tools::isXml("   "));
    EXPECT_FALSE(Tools::isXml("\r\n\t "));
    EXPECT_FALSE(Tools::isXml("\xEF\xBB\xBF"));

    // Plain text
    EXPECT_FALSE(Tools::isXml("plain text"));
    EXPECT_FALSE(Tools::isXml("some text with <tags/> inside"));

    // SVS-style key-value image description
    EXPECT_FALSE(Tools::isXml("Aperio Image Library v11.2.1\r\n"
                              "46920x33014 [0,100 46000x32893] (256x256) JPEG/RGB Q=30"));

    // JSON is not XML
    EXPECT_FALSE(Tools::isXml("{\"root\": \"value\"}"));

    // Malformed markup
    EXPECT_FALSE(Tools::isXml("<"));
    EXPECT_FALSE(Tools::isXml("<root"));
    EXPECT_FALSE(Tools::isXml("<root>"));
    EXPECT_FALSE(Tools::isXml("<root></other>"));
    EXPECT_FALSE(Tools::isXml("<root><child></root>"));
    EXPECT_FALSE(Tools::isXml("<root attr=\"unclosed></root>"));
    EXPECT_FALSE(Tools::isXml("<!-- unterminated comment"));

    // No root element
    EXPECT_FALSE(Tools::isXml("<?xml version=\"1.0\"?>"));
    EXPECT_FALSE(Tools::isXml("<!-- only a comment -->"));

    // A closing tag alone is not a document
    EXPECT_FALSE(Tools::isXml("</root>"));

    // Embedded NUL characters must not truncate the check
    EXPECT_FALSE(Tools::isXml(std::string("\0<root/>", 8)));
}
