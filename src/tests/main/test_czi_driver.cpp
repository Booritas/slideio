...
TEST(CZIImageDriver, readBlock4D)
{
    ...
    for (int channelIndex = 0; channelIndex < 3; ++channelIndex)
    {
        ...
        cv::Mat raster;
        std::vector<int> channelIndices = {channelIndex};
        scene->readResampled4DBlockChannels(sceneRect, sceneRect.size(), channelIndices, {2, 5}, {0, 1}, raster);
        ...
        for (int zSliceIndex = zSliceRange.start; zSliceIndex < zSliceRange.end; ++zSliceIndex)
        {
            ...
            std::string bmpFileName = std::string("pJP31mCherry.grey/pJP31mCherry_b0t0z") +
                                      std::to_string(zSliceIndex) + std::string("c") + std::to_string(channelIndex) +
            ...
            cv::extractChannel(bmpImage, channelImage, 0);
            int compare =
                std::memcmp(sliceRaster.data, channelImage.data, sliceRaster.total() * sliceRaster.elemSize());
            EXPECT_EQ(compare, 0);
        }
    }
}
...
{
    {
        ...
        int values2[6] = {0};
        uint64_t sceneId =
            slideio::CZIScene::sceneIdFromDims(values[0], values[1], values[2], values[3], values[4], values[5]);
        slideio::CZIScene::dimsFromSceneId(sceneId, values2[0], values2[1], values2[2], values2[3], values2[4],
                                           values2[5]);
...
{
    {
        std::vector<slideio::Dimension> dims = {
            {'V', 1, 1}, {'H', 2, 1}, {'I', 3, 1}, {'R', 4, 1}, {'B', 5, 1}, {'S', 6, 1},
        };
    ...
    {
        std::vector<slideio::Dimension> dims = {
            {'V', 1, 1}, {'H', 2, 1}, {'I', 3, 1}, {'R', 4, 1}, {'B', 5, 1}, {'S', 6, 2},
        };
        std::vector<uint64_t> ids;
        slideio::CZIScene::sceneIdsFromDims(dims, ids);
    ...
    {
        std::vector<slideio::Dimension> dims = {
            {'V', 1, 2}, {'H', 2, 2}, {'I', 3, 2}, {'R', 4, 2}, {'B', 5, 2}, {'S', 6, 2},
        };
        std::vector<uint64_t> ids;
        slideio::CZIScene::sceneIdsFromDims(dims, ids);
...
{
    const std::string images[] = {"03_14_2019_DSGN0545_A_wb_1353_fov_1_633.czi", "pJP31mCherry.czi"};
    slideio::CZIImageDriver driver;
...
{
    const std::string images[] = {"03_14_2019_DSGN0545_A_wb_1353_fov_1_633.czi", "pJP31mCherry.czi", "test2.czi"};
...
TEST(CZIImageDriver, zoomLevels)
{
    ...
    for (int levelIndex = 0; levelIndex < numLevels; ++levelIndex)
    {
        ...
        if (levelIndex == 0) EXPECT_EQ(level->getSize(), slideio::Tools::cvSizeToSize(rect.size()));
    }
}
