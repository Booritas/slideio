...
std::string TestTools::getTestImageDirectory(bool priv)
{
    ...
    if (var == nullptr) RAISE_RUNTIME_ERROR << "Undefined environment variable: " << varName;
...
std::string TestTools::getTestImagePath(const std::string& subfolder, const std::string& image, bool priv)
{
    ...
    if (!subfolder.empty()) imagePath += std::string("/") + subfolder;
...
void TestTools::writeRawImage(const std::string& path, const cv::Mat& image)
{
    ...
    if (!os.is_open()) RAISE_RUNTIME_ERROR << "File " << path << " could not be opened for writing";
...
void TestTools::writePNG(cv::Mat raster, const std::string& filePath)
{
    ...
    if (!fp) RAISE_RUNTIME_ERROR << "File " << filePath << " could not be opened for writing";
    ...
    try
    {
        ...
        if (!pngStruct) RAISE_RUNTIME_ERROR << "png_create_write_struct failed";

        png_infop pngInfo = png_create_info_struct(pngStruct);
        if (!pngInfo) RAISE_RUNTIME_ERROR << "png_create_info_struct failed";

        if (setjmp(png_jmpbuf(pngStruct))) RAISE_RUNTIME_ERROR << "Error during init_io";

        png_init_io(pngStruct, fp);

        /* write header */
        if (setjmp(png_jmpbuf(pngStruct))) RAISE_RUNTIME_ERROR << "Error during writing header";
        ...
        png_set_IHDR(pngStruct, pngInfo, width, height, bitDepth, colorType, PNG_INTERLACE_NONE,
                     PNG_COMPRESSION_TYPE_BASE, PNG_FILTER_TYPE_BASE);

        png_write_info(pngStruct, pngInfo);
        if (littleEndian() && png_get_bit_depth(pngStruct, pngInfo) > 8) png_set_swap(pngStruct);

        /* write bytes */
        if (setjmp(png_jmpbuf(pngStruct))) RAISE_RUNTIME_ERROR << "Error during writing bytes";
        if (!raster.isContinuous()) RAISE_RUNTIME_ERROR << "Continuous raster expected";
        ...
        /* end write */
        if (setjmp(png_jmpbuf(pngStruct))) RAISE_RUNTIME_ERROR << "Error during end of write";
...
void TestTools::readPNG(const std::string& filePath, cv::OutputArray output)
{
    ...
    if (!fp) RAISE_RUNTIME_ERROR << "File " << filePath << " could not be opened for reading.";
    ...
    try
    {
        ...
        if (!pngStruct) RAISE_RUNTIME_ERROR << "[read_png_file] png_create_read_struct failed";

        png_infop pngInfo = png_create_info_struct(pngStruct);
        if (!pngInfo) RAISE_RUNTIME_ERROR << "[read_png_file] png_create_info_struct failed";

        if (setjmp(png_jmpbuf(pngStruct))) RAISE_RUNTIME_ERROR << "[read_png_file] Error during init_io";
        ...
        /* read file */
        if (setjmp(png_jmpbuf(pngStruct))) RAISE_RUNTIME_ERROR << "[read_png_file] Error during read_image";
...
void TestTools::readTiffDirectories(const std::string& filePath, const std::vector<int>& dirIndices,
...
void TestTools::multiThreadedTest(const std::string& filePath, slideio::ImageDriver& driver, int numberRois,
                                  int numThreads)
{
...
std::shared_ptr<slideio::CVScene> TestTools::findScene(std::shared_ptr<slideio::CVSlide> slide, const std::string& name)
{
    ...
    for (int i = 0; i < numScenes; ++i)
    {
        ...
        if (sceneName == name) return scene;
    }
