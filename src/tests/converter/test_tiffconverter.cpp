...
{
    uint8_t color = static_cast<uint8_t>(std::min(255, frame * 20 + slice * 5 + channel));
    return color;
}
...
public:
    DummyScene() = default;
    std::string getChannelName(int channel) const override { return ::getChannelName(channel); }
...
static void checkTiff(const TiffConverter& converter, const std::string& filePath,
...
static void testOMETIFFSubset(const ConverterParameters& params, const std::shared_ptr<CVScene>& scene,
                              bool interleaved3Channels = false)
{
    ...
    if (std::filesystem::exists(outputPath)) std::filesystem::remove(outputPath);
    {
        ...
        auto tiffParams = std::static_pointer_cast<const TIFFContainerParameters>(params.getContainerParameters());
...
TEST(TiffConverterTests, jpegToOMETIFF)
{
    ...
    auto tiffParams = std::static_pointer_cast<TIFFContainerParameters>(parameters.getContainerParameters());
    ...
    int progressLast = 0;
    ASSERT_NO_THROW(converter.createTiff(
        outputPath,
        [&progress, &progressLast](int pr) {
            progressLast = pr;
            progress.insert(pr);
        },
        1));
    ...
    std::vector<cv::Size> dirSizes = {
        cv::Size(sceneWidth / 2, sceneHeight / 2), cv::Size(sceneWidth / 4, sceneHeight / 4),
        cv::Size(sceneWidth / 8, sceneHeight / 8), cv::Size(sceneWidth / 16, sceneHeight / 16)};
    int dirIndex = 0;
...
TEST(TiffConverterTests, jpegToSVS)
{
    ...
    std::vector<cv::Size> dirSizes = {cv::Size(sceneWidth, sceneHeight), cv::Size(sceneWidth / 2, sceneHeight / 2),
